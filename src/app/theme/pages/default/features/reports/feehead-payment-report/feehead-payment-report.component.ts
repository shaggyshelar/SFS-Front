...
export class FeeheadPaymentReportComponent implements OnInit {
    ...
    onSearchReport() {
        ...
        if (this.startDate && this.endDate) {
            if (this.startDate < this.endDate) {
                ...
                this.getAllFeeheadPaymentReport(this.url);
    ...
    getAllFeeheadPaymentReport(url) {
        Helpers.setLoading(true);
        this.invoiceService.getFeeheadPaymentReport(this.url).subscribe(
            response => {
                Helpers.setLoading(false);
    ...
    calculateGroupTotal(feeHeadName: string) {
        ...
        if (this.schoolList) {
            for (let school of this.schoolList) {
                if (school.feeHeadName === feeHeadName) {
                    total += school.amount;
                }
            }
        }
