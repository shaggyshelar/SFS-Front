...
export class CommonService {
  ...
  getAllAudit(url) {
    return this.http.get(AppSettings.API_ENDPOINT + 'schools/'+(localStorage.getItem('schoolId') === undefined ? 0 : localStorage.getItem('schoolId'))+'/Vwauditlogs' + url, AppSettings.requestOptions()).map((response: Response) => response.json());
  }
  getAuditCount(url) {
    return this.http.get(AppSettings.API_ENDPOINT + 'schools/'+(localStorage.getItem('schoolId') === undefined ? 0 : localStorage.getItem('schoolId'))+'/Vwauditlogs/count' + url, AppSettings.requestOptions()).map((response: Response) => response.json());
  }
